...
if (request()->id != 'ASC' && request()->id != 'DESC') {
    ...
    if (!empty($strArray[4]) && !empty($strArray[5])) {
        Session::put($cookie_name, [$strArray[4], $strArray[5]]);
                                        ...
                                        @if (!empty(Session::get($cookie_name)))
                                            @if (is_array(Session::get($cookie_name)) == 1)
                                                ...
                                                <input type="hidden" id="cat_name"
                                                    value="{{ Session::get('request_id.0') }}">
                                            @else
                                                {{ Session::get($cookie_name) }}
                                                <input type="hidden" id="cat_name"
...
if (empty($anime_cat[0]) || !isset($anime_cat[0])) {
                        ...
                        <div class="col-lg-8 col-md-8 col-sm-8 ">
                            <div class="product__item">
                                <div class="product__item__pic set-bg" style="height:235px !important;"
                                    data-setbg="{{ Storage::url('img/anime') }}/not_found.gif">
...
?>
                        @foreach ($anime_cat as $anime)
                            <div class="col-lg-4 col-md-6 col-sm-6">
                                <div class="product__item">
                                    <a
                                        href="{{ url('/') }}/anime-detail/{{ $anime->anime_id }}?{{ $anime->anime_title }}">
                                        <div class="product__item__pic set-bg" style="height:235px !important;"
                                            data-setbg="{{ Storage::url('img/anime') }}/{{ $anime->anime_image }}">
                                            {{-- <div class="ep">18 / 18</div>
                                        <div class="comment"><i class="fa fa-comments"></i> 11</div>
                                        <div class="view"><i class="fa fa-eye"></i> 9141</div> --}}
                                        </div>
...
<script>
    ...
    $("select#order_by").on("change", function() {
        ...
        if ($.trim(cat_name2).length == 0 && $.trim(order_by2).length !== 0) {
            ...
            ChangeUrl("Anime| template", "{{ url('category/') }}/" + cat_name + "?name=" + cat_name +
                "&order_by=" + order_by)
            changeurl2()
